...
class ReservationController extends Controller
{
    ...
    public function download(Ticketinstance $ticketInstance)
    {
        ...
        $evenement = $ticketInstance->reservation->ticket->typeticket->evenement;
        ...
        if ($evenement->image && Storage::exists($evenement->image)) {
            try {
                ...
                $imageContent = Storage::get($evenement->image);
                $mimeType = Storage::mimeType($evenement->image);

                // Convertir en Base64 Data URL
                $backgroundUrl = 'data:' . $mimeType . ';base64,' . base64_encode($imageContent);
                
            } catch (\Exception $e) {
                // Gérer l'erreur de lecture du fichier
                // Vous pouvez logguer l'erreur ou laisser $backgroundUrl à null
            }
        }
        ...
        $options = [
            ...
            'isRemoteEnabled' => true, // Important si le SVG dépend de ressources externes (moins probable ici, mais utile)
        ];
        
        // Génération du PDF à partir de la vue Blade
        $pdf = PDF::loadView('welcome', $data)->setOptions($options);
    ...
    public function allTicketPdf()
    {
        $ticketInstances = Ticketinstance::all();

        $pdf = PDF::loadView('welcome', compact('ticketInstances'));

        return $pdf->download('mes_billets.pdf');
    }
}
